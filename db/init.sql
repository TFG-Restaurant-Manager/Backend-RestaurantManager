...
CREATE TABLE employee (
	...
	CONSTRAINT uq_employee_restaurant UNIQUE (code, restaurant_id),
...
CREATE TABLE work_schedules (
	...
	CONSTRAINT fk_ws_employee FOREIGN KEY (employee_code)
		REFERENCES employee(code) ON DELETE CASCADE ON UPDATE CASCADE,
...
BEGIN
  ...
  NEW.code := prefix_val || LPAD((NEXTVAL('employee_code_seq'))::text, 5, '0');
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
SELECT
	er.id AS employee_restaurant_id,
	e.id AS employee_id,
...
JOIN employees e ON er.employee_dni = e.id
JOIN roles r ON er.role_id = r.id
LEFT JOIN work_schedules ws ON er.id = ws.employee_restaurant_dni;
